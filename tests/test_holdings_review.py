...
def _patch_env(score_total):
    ...
    hr.REVIEW_FILE = os.path.join(
        TMP, f"review_{uuid.uuid4().hex[:8]}.json"
    )  # 每测独立——防顺序污染
    path = _mk_portfolio_with_holding()
    real_portfolio = hr.pf.Portfolio  # patch 前保存原类（避免 lambda 自递归）
    patches = [
        patch(
            "tools.strategy_engine.portfolio.Portfolio",
            lambda p=path: real_portfolio(p),
        ),
        patch(
            "tools.strategy_engine.holdings_review.data.tencent_quote",
            lambda codes: {
                "600036": {"code": "600036", "name": "招商银行", "price": 38.0}
            },
        ),
        patch(
            "tools.strategy_engine.holdings_review._score_position",
            lambda code, q: SimpleNamespace(total=score_total),
        ),
        patch(
            "tools.strategy_engine.holdings_review.ms.market_status",
            lambda: {"status": "正常"},
        ),
