...
def main():
    ...
    print(f"{'排名':<4}{'代码':<8}{'名称':<10}{'总分':<7}{'涨跌幅%':<9}{'上榜后1日%':<10}{'上榜后2日%'}")
    print('-' * 62)
    for i, (s, r) in enumerate(scored[:10], 1):
        d1 = r.get('上榜后1日')
        d2 = r.get('上榜后2日')
        print(f"{i:<4}{r['代码']:<8}{r['名称']:<10}{s['total']:<7}{r['涨跌幅']:<9}"
    ...
    highs = [r['上榜后1日'] for s, r in scored[:10] if r.get('上榜后1日') == r.get('上榜后1日')]
    lows = [r['上榜后1日'] for s, r in scored[-10:] if r.get('上榜后1日') == r.get('上榜后1日')]
    if highs and lows:
