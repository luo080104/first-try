...
def get_fundamentals(code: str, price: float = 0.0,
                    debt_exempt: bool = False) -> dict[str, Any]:
    """基本面字段（日级缓存）——价值面 40 分输入"""
    now = time.time()
    if code in _cache and now - _cache[code][0] < _CACHE_TTL:
        return _cache[code][1]

    f: dict[str, Any] = {"roe": 0, "sales_margin": 0, "debt_ratio": 0,
                         "ocf_gt_profit": False, "dividend_yield": 0, "growth_ok": False,
    ...
    try:
        ...
        if lrb:
            ...
            profit = _to_float(lrb[0].get("净利润") or lrb[0].get("归属于母公司所有者的净利润"))
            if rev > 0:
                f["sales_margin"] = round(profit / rev * 100, 1)
            growth = _to_float(lrb[0].get("营业总收入_同比") or lrb[0].get("营业收入_同比"))
            if growth > 5:
                f["growth_ok"] = True
        if fzb:
        ...
        equity = _to_float(fzb[0].get("归属于母公司股东权益合计")
                           or fzb[0].get("归属于母公司所有者权益合计")
                           or fzb[0].get("所有者权益合计")) if fzb else 0
        if lrb and equity > 0:
