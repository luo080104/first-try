...
CATEGORY_HINTS = {
    ...
    '食品': ['牛奶', '奶制品', '奶茶', '坚果', '零食', '咖啡', '茶叶', '酸奶', '巧克力', '螺蛳粉', '面包', '蛋糕',
    ...
    '日用百货': ['洗衣', '纸', '垃圾袋', '洗洁精', '湿巾', '杯', '毛巾', '收纳', '牙刷', '洗发',
                ...
                '狗粮', '猫砂', '奶粉', '奶瓶', '尿不湿', '纸尿裤', '四件套', '枕头', '窗帘', '地毯', '玩具',
...
def query_items(keyword: str = '', category: str = '', platform: str = '',
    ...
    if total == 0 and keyword:
        ...
        import re
        _STOP = ('帮我看', '帮我', '我看', '的电脑', '买', '一个', '想要', '大概', '什么',
...
# ========== v6 多用户：家庭品类库（15 细品类 → 大品类 + 采集词）==========

FAMILY_CATEGORIES = [
    # (细品类名, 大品类, [采集词...])
    ('女士服装', '服饰', ['连衣裙', '女装', '半身裙', '打底衫', '阔腿裤', '针织衫', '防晒衣']),
    ('男士服装', '服饰', ['男装', '男士外套', '男士衬衫', '休闲裤', '夹克', '男士卫衣']),
    ('运动户外', '服饰', ['运动鞋', '跑步鞋', '瑜伽垫', '冲锋衣', '羽毛球拍', '篮球']),
    ('护肤品', '日用百货', ['面霜', '精华', '爽肤水', '防晒霜', '面膜', '洗面奶']),
    ('美妆', '日用百货', ['口红', '粉底液', '眼影', '眉笔', '定妆喷雾']),
    ('个护清洁', '日用百货', ['洗发水', '沐浴露', '电动牙刷', '吹风机', '剃须刀', '洗衣液']),
    ('食品生鲜', '食品', ['纯牛奶', '坚果', '零食', '咖啡', '茶叶', '酸奶', '巧克力', '螺蛳粉']),
    ('母婴', '日用百货', ['奶粉', '纸尿裤', '婴儿推车', '婴儿湿巾', '辅食']),
    ('智能设备', '数码家电', ['智能手表', '扫地机器人', '智能门锁', '无线耳机', '智能音箱', '行车记录仪']),
    ('数码', '数码家电', ['手机', '笔记本电脑', '平板', '显示器', '机械键盘', '显卡']),
    ('家电', '数码家电', ['冰箱', '洗衣机', '空调', '电饭煲', '空气炸锅', '吸尘器', '电风扇']),
    ('家居', '日用百货', ['四件套', '枕头', '收纳箱', '窗帘', '地毯']),
    ('宠物', '日用百货', ['猫粮', '狗粮', '猫砂', '宠物零食', '牵引绳']),
    ('图书', '日用百货', ['小说', '童书', '教辅', '历史书', '科幻']),
    ('日用百货', '日用百货', ['抽纸', '垃圾袋', '洗洁精', '湿巾', '保温杯']),
]


def ensure_family_tasks():
    """家庭品类采集词入库（source='family'，幂等）"""
    conn = get_conn()
...
def log_search(user_name: str, keyword: str, category: str = ''):
    """记录一次搜索（INSERT OR REPLACE：同用户同词只留最近一条，靠唯一索引，WorkBuddy P1-3）"""
    conn = get_conn()
    conn.execute('INSERT OR REPLACE INTO search_history (user_name, keyword, category, searched_at) VALUES (?,?,?,datetime(\'now\',\'localtime\'))',
