...
@_crawl_single
async def run_crawl_round(
    pages: int = 2, max_seconds: int = 72000
) -> dict:  # 20h兜底（小骆：不要设限，词跑完自然停）
    ...
    try:
        ...
        for task in tasks:
            ...
            if elapsed_now > max_seconds:
                print(
                    f"[crawl] ⏰ 已达时长上限 {max_seconds // 3600} 小时，停止（剩余 {len(tasks) - (ok_count + fail_count)} 词下轮继续）"
                )
                with _lock:
                    _progress["errors"].append(
                        f"[时长上限] 跑满 {max_seconds // 3600} 小时自动停止，剩余词下轮继续"
                    )
                break
            ...
            try:
                ...
                print(
                    f"[crawl] ✅ {kw}: +{added} 件（累计 {len(items)} 条，耗时 {time.time() - w0:.0f}s）"
                )
            except Exception as e:
    ...
    return {
        ...
        "done": ok_count,
        "failed": fail_count,
        "new_items": new_total + jd_added,
        "new_words": new_words[:20],
        "added_words": added_words,
        "elapsed": int(time.time() - t0),
        "stopped_by_timeout": (ok_count + fail_count) < len(tasks)
